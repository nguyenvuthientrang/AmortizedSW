...
import models
import datasets
from functions import train_swg, validate, LinearLrDecay, load_params, copy_params
...
def main():
    ...
    fids=[]
    iscores=[]
    lr_schedulers = (gen_scheduler) if args.lr_decay else None
    for epoch in tqdm(range(int(start_epoch), int(args.max_epoch)), desc='total progress'):
        train_swg(args, gen_net, slicer, gen_optimizer, gen_avg_param, train_loader, epoch, writer_dict,
              lr_schedulers)
